...
const FROM_EMAIL     = "ZAP-CASH <onboarding@resend.dev>";
const SITE_URL       = "https://zipinnovate.com";

// Owner email for test mode (Resend free tier only sends to this address)
...
async function sendEmail(to, subject, html) {
  ...
  // If domain not verified, only send to owner email (Resend test restriction)
  const recipient = DOMAIN_VERIFIED ? to : OWNER_EMAIL;
  if (!DOMAIN_VERIFIED && to !== OWNER_EMAIL) {
    console.log(`[Email] Domain not verified — would send to ${to}, sending to owner instead`);
  }
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: FROM_EMAIL, to: recipient, subject: DOMAIN_VERIFIED ? subject : `[For: ${to}] ${subject}`, html }),
    });
